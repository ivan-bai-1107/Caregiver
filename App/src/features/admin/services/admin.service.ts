...
export async function listAdminReviewComments(
  params: { status?: CommunityReviewStatus; page?: number; pageSize?: number } = {},
) {
  return apiClient.get<PagedResponse<CommunityComment>>("/api/admin/reviews/comments", params, adminOptions());
}

export async function updateAdminReviewComment(commentId: string, payload: AdminReviewUpdatePayload) {
  return apiClient.put<CommunityComment>(`/api/admin/reviews/comments/${commentId}`, payload, adminOptions());
}
