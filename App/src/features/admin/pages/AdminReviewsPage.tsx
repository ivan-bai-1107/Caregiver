...
import { adminReviewStatusLabels } from "@/features/admin/model";
import { useAdminReviewsState, type AdminReviewKind } from "@/features/admin/state/useAdminReviewsState";
...
function getReviewTitle(item: AdminReviewItem) {
  return item.type === "post" ? item.item.title : `评论：${item.item.content.slice(0, 36)}`;
}

function getReviewContent(item: AdminReviewItem) {
  return item.type === "post" ? item.item.content : item.item.content;
}

function getReviewMeta(item: AdminReviewItem) {
  return item.type === "post" ? getCommunityTagLabel(item.item.tag) : `帖子 ${item.item.postId.slice(0, 8)}`;
}
...
export function AdminReviewsPage() {
  ...
  const kindTabs: Array<{ key: AdminReviewKind; label: string }> = [
    { key: "posts", label: "帖子" },
  ...
  return (
    <div className="p-8">
          ...
          {kindTabs.map((tab) => (
            <button
              ...
              className={`px-5 py-2.5 rounded-xl text-sm transition-colors ${
                kind === tab.key ? "bg-primary text-white" : "bg-white text-gray-600 border border-gray-200 hover:bg-gray-50"
              }`}
            >
              {tab.label}
            </button>
      ...
      <div className="space-y-4">
        {items.map((review) => {
          const Icon = review.type === "post" ? FileText : MessageSquare;
          return (
            <div key={`${review.type}-${review.item.id}`} className="bg-white rounded-2xl p-6 border border-gray-200">
              <div className="flex items-start gap-4">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${
                  review.type === "post" ? "bg-blue-50 text-blue-600" : "bg-purple-50 text-purple-600"
                }`}>
          ...
          <div className="bg-white rounded-2xl p-8 w-full max-w-2xl shadow-xl max-h-[80vh] overflow-y-auto" onClick={(event) => event.stopPropagation()}>
            <div className="flex items-center gap-2 mb-4">
              <span className="px-2.5 py-1 text-xs rounded-lg bg-primary/10 text-primary">
                {previewItem.type === "post" ? "社区帖子" : "社区评论"}
              </span>
