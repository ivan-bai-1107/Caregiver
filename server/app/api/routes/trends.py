...
from app.services.trend_service import get_metric_trend, get_trend_analysis
...
@router.get("/{patient_id}/metrics/trend-analysis")
def read_metric_trend_analysis(
    patient_id: str,
    db: Annotated[Session, Depends(get_db)],
    current_user: Annotated[User, Depends(get_current_user)],
    metric_type: Annotated[str, Query(alias="metricType")],
    ...
    return success_response(get_trend_analysis(db, current_user, patient_id, metric_type, start_at, end_at))
