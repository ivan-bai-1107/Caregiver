...
真实发送 QQ 邮箱验证码时，在本地 `server/.env` 配置：
...
`SMTP_PASSWORD` 是 QQ 邮箱 SMTP 授权码，不是 QQ 密码。不要提交 `server/.env`，不要把邮箱授权码或 DeepSeek key 写入 README、测试脚本、前端环境变量或代码。
...
- 不要提交 `server/.env`，真实 API key 只能放本地环境变量。
