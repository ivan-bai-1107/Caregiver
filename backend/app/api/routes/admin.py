...
def read_review_comments(
    ...
    return success_response(list_review_comments(db, status_filter, page, page_size))


@router.put("/reviews/comments/{comment_id}")
def update_review_comment_endpoint(
    comment_id: str,
    payload: AdminReviewUpdate,
    db: Annotated[Session, Depends(get_db)],
    _: Annotated[AdminUser, Depends(get_current_admin)],
) -> dict[str, object]:
    return success_response(update_review_comment(db, comment_id, payload))
