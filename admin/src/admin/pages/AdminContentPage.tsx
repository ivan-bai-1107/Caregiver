...
import type { AdminArticleStatus, AdminKnowledgeArticle, AdminKnowledgeArticleDraft } from "@admin/model";
import { adminArticleStatusLabels } from "@admin/model";
import { useAdminContentState } from "@admin/state/useAdminContentState";
import { formatDateTimeLabel } from "@/shared/lib/date";

const coverColorOptions = [
  { value: "primary", label: "护理绿" },
  { value: "blue", label: "安心蓝" },
  { value: "accent", label: "提示橙" },
  { value: "warm", label: "暖色" },
];

const coverColorValues = new Set(coverColorOptions.map((option) => option.value));

function getReadableAuthorName(authorName: string) {
  const value = authorName.trim();
  if (!value || coverColorValues.has(value)) {
    return "护理知识编辑";
  }
  return value;
}
          ...
          <tbody>
            {articles.map((article) => (
              <tr key={article.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                ...
                <td className="py-4 px-6 text-sm text-gray-600">{getReadableAuthorName(article.authorName)}</td>
